            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                if (result > 0)
                {
                    var claims = new[]
                    {
                            ...
                            new Claim("CurrencyPreference",request.CurrencyPreference),
                            new Claim("UserId", request.PersonId.ToString())
                        };
                    ...
                    return new OkObjectResult(new { Message = "User udpated successfully",token =  new JwtSecurityTokenHandler().WriteToken(token)});
                }
                else
                {
